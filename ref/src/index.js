...
async function main() {
  ...
  const deviceCount = parseInt(await prompt(chalk.yellow("How many device you want?: ")));
  ...
  try {
    for (let i = 0; i < count; i++) {
      ...
      try {
        const deviceHashes = Array.from({ length: deviceCount }, () => generateDeviceHash());
      ...
      logMessage(i + 1, count, "Waiting 1 seconds for next account", "process");
    ...
    console.log(chalk.magenta("\n[*] Completed all accounts!"));
    console.log(chalk.green("[*] Account ref in accounts.txt and accounts.json"));
    rl.close();
  }
}
